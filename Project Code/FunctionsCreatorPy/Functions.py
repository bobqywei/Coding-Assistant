...
#GOAL: "greater than or equal to", *****not operator on pre boolean expression (if i see not and there isn't an equals after it, then we look for thte boolean expression follwing not), **for,while, data type, print (w/ doubles) and and/or.. also import calculator's constants and function to evaluate single word numbers or stuff like that
	# **add arrys, add length, FIX FILE OUTPUT
	...
	#DONEnext steps: line 173, instead of indexing for endprint, what if print and endprint are on two diff lines? then we want to kepe reading input line after line until endprint shows up
	#IMPLY AND KEYWORDS i.e. check after the several if/elifs a separte if if it's in this array. if so, check if there is an and preceding this keyword. no? add a and
	#ALSO: if we have no blocks at all, want end brace/loop/if i.e. END CODE to close with semi colon only, no brace. but this shouldnt happen due to class creation..
	fileObj = None
	string = ""
	stringArray = []
	currentArray = None
				...
				#ANOTHER POSSIBLE FIX IS TO JUST EXCLUDE IT ENTIRELY, BUT IS THIS EFFICIENT?
					...
					for j in range(ind, len(self.currentArray)):
						if self.currentArray[j] in self.BOOLSYM and (self.currentArray[j+1] in self.BOOLSYM or self.currentArray[j] == "<" or self.currentArray[j] == ">"):
							tempCumulative = self.stringArray
				...
				#similar to and, if we see a boolean expression follow then we automatically know that preceding isi also boolean. if we find keyword or other symbol first, then do nothing
					...
					for j in range(ind, len(self.currentArray)):
						if self.currentArray[j] in self.BOOLSYM and (self.currentArray[j+1] in self.BOOLSYM or self.currentArray[j] == "<" or self.currentArray[j] == ">"):
							...
							break
			elif (self.currentArray[ind] in self.KEYWORDS or self.currentArray[ind] in self.INDEPDATALIST): #cases of remaining keywords or var names that aren't specifically handled. i.e. we for certain want to store just the current index into the new array
